...
func testJetsInsertWhitelist(t *testing.T) {
	...
	tx := MustTx(boil.Begin())
	defer tx.Rollback()
	if err = jet.Insert(tx, jetColumnsWithoutDefault...); err != nil {
		t.Error(err)
	}
...
func testJetToOnePilotUsingPilot(t *testing.T) {
	...
	if err = local.L.LoadPilot(tx, false, (*[]*Jet)(&slice)); err != nil {
		t.Fatal(err)
	}
...
var (
	jetDBTypes = map[string]string{`Age`: `int`, `Color`: `text`, `ID`: `int`, `Name`: `text`, `PilotID`: `int`}
	_          = bytes.MinRead
)
...
func testJetsUpsert(t *testing.T) {
	...
	tx := MustTx(boil.Begin())
	defer tx.Rollback()
	if err = jet.Upsert(tx, nil); err != nil {
